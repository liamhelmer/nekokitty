...
class CodexCommandTests(unittest.TestCase):
    ...
    def test_web_search_uses_luna_low_plan_read_only_and_search(self) -> None:
        ...
        self.assertIn("cute, motherly", prompt)
        self.assertIn("orange cat-shaped car", prompt)
        self.assertIn("gummy worms", prompt)
        self.assertIn("personality changes presentation, never evidence", prompt)
        self.assertIn("[meow]", prompt)
    ...
    def test_story_uses_terra_low_yolo_with_tightly_scoped_prompt(self) -> None:
        ...
        self.assertIn("Neko's warm mischievous storytelling voice", prompt)
        self.assertIn("Cat-sound markers belong only", prompt)
        self.assertEqual(argv[-1], "-")
